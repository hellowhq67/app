...
async function verifyStrictScoring() {
    ...
    const raPerfect = await scorePteAttemptV2('read_aloud', {
    ...
    console.log(`> Features: ${JSON.stringify(raPerfect.check_results)}`);

    // Case B: Missing Words (Penalty)
    console.log('Case B: Missing "particularly specifically"');
    const raMissing = await scorePteAttemptV2('read_aloud', {
    ...
    const essayShort = await scorePteAttemptV2('write_essay', {
        submission: { text: "Technology is good. It helps us work faster." },
        questionContent: prompt,
        mode: 'mock'
    });
    console.log(`> Form Score (Length): ${essayShort.form?.score} (Expected Low/0)`);
