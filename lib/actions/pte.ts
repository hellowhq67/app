...
export async function submitAttempt(data: {
  ...
  // Transcribe audio if transcript is empty
  let transcript = data.transcript
  if (!transcript || transcript.trim() === '') {
    try {
      const { getTranscriber } = await import('@/lib/pte/transcription')
      const transcriber = await getTranscriber()
      const transcriptionResult = await transcriber.transcribe({ audioUrl: data.audioUrl })
      transcript = transcriptionResult.transcript || ''
      console.log('[submitAttempt] Transcription successful')
    } catch (error) {
      console.warn('[submitAttempt] Transcription failed, continuing with empty transcript:', error)
      transcript = ''
    }
  }
  ...
  try {
    ...
    // Convert AI feedback to score format
    aiScore = {
      ...
      feedback: feedback.suggestions?.join(' ') || 'AI scoring complete',
    }
