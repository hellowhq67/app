...
/**
 * Get subscription by SSL Commerz transaction ID
 */
export async function getSubscriptionBySSLCommerzTranId(
    tranId: string
): Promise<Subscription | null> {
    const result = await db.query.subscriptions.findFirst({
        where: eq(subscriptions.sslCommerzTranId, tranId),
    });
    return result || null;
}

/**
 * Get transaction by SSL Commerz transaction ID
 */
export async function getTransactionBySSLCommerzTranId(
    tranId: string
): Promise<Transaction | null> {
    const result = await db.query.transactions.findFirst({
        where: eq(transactions.sslCommerzTranId, tranId),
    });
    return result || null;
}

/**
 * Create SSL Commerz transaction record
 */
export async function createSSLCommerzTransaction(data: {
    userId: string;
    tier: string;
    amount: number;
    currency: string;
    ...
    const [transaction] = await db.insert(transactions).values({
        ...
        description: `${data.tier} subscription purchase`,
        sslCommerzTranId: data.tranId,
...
/**
 * Update SSL Commerz transaction status
 */
export async function updateSSLCommerzTransactionStatus(
    tranId: string,
