  ...
  return (
    ...
    >
      ...
      <Link href="/" className="flex items-center gap-2 absolute left-6">
        ...
        <span className="hidden sm:block font-bold text-sm tracking-tight text-foreground/90">
          Pedagogist
        </span>
      </Link>
      ...
      <div className="hidden md:flex items-center gap-2 text-sm font-medium mx-auto">
        <Popover>
          ...
          <PopoverContent
            align="center"
            className="w-[40rem] p-5 shadow-xl border-border/50 bg-background/95 backdrop-blur-sm"
          >
            <div className="grid grid-cols-3 gap-6">
              {menu.map((section) => (
                <div key={section.title}>
                  <div className="text-xs uppercase text-muted-foreground mb-3 font-bold tracking-wider">
                    {section.title}
                  </div>
                  <div className="flex flex-col gap-1.5">
                    {section.items.map(({ href, label, icon: Icon }) => (
                      <Link
                        key={href}
                        href={href}
                        className="text-sm p-2.5 -mx-2.5 hover:bg-accent/80 rounded-lg flex items-center gap-2.5 text-foreground/80 hover:text-foreground transition-all duration-200 group"
                      >
      ...
      <div className="flex items-center gap-3 ml-auto">
        ...
        {user ? (
          <DropdownMenu>
            ...
            <DropdownMenuContent
              align="end"
              className="w-56 p-2 shadow-xl border-border/50"
            >
              ...
              <DropdownMenuItem
                asChild
                className="p-2 cursor-pointer rounded-md focus:bg-accent"
              >
          ...
          <>
            <Button
              ...
              variant="ghost"
              size="sm"
              className="hidden sm:inline-flex rounded-full px-5 text-muted-foreground hover:text-foreground font-medium"
            >
            ...
            <Button
              asChild
              size="sm"
              className="rounded-full bg-blue-600 hover:bg-blue-700 text-white border-0 shadow-sm px-5 font-semibold transition-all hover:scale-105 active:scale-95"
            >
        ...
        <Sheet>
          <SheetTrigger asChild>
            <Button
              variant="ghost"
              size="icon"
              className="md:hidden rounded-full h-9 w-9 hover:bg-accent"
            >
          ...
          <SheetContent
            side="top"
            className="w-full pt-6 pb-6 rounded-b-[2rem] border-b border-border shadow-2xl"
          >
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                {navItems.map((item) => (
                  <Link
                    key={item.link}
                    href={item.link}
                    className="flex items-center p-3 rounded-xl hover:bg-accent/50 transition-colors border border-transparent hover:border-border"
                  >
                ...
                <Link
                  href="/dashboard"
                  className="flex items-center p-3 rounded-xl bg-blue-50 dark:bg-blue-950/20 text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-blue-900/50"
                >
              ...
              {user && (
                <form action={signOutAction} className="mt-2">
                  <Button
                    variant="outline"
                    type="submit"
                    className="w-full justify-center h-12 text-destructive hover:text-destructive hover:bg-destructive/5 border-destructive/20 text-base font-medium rounded-xl"
                  >
