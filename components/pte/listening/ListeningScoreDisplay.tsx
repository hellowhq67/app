...
export default function ListeningScoreDisplay({
  scoreData,
}: {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>AI Listening Analysis</CardTitle>
        <CardDescription>
          Here's the AI-powered feedback on your listening performance.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <div className="space-y-4 rounded-lg border p-4">
            ...
            {scoreData.content && <ScorePill label="Content" {...scoreData.content} />}
            {scoreData.accuracy && <ScorePill label="Accuracy" {...scoreData.accuracy} />}
            {scoreData.grammar && <ScorePill label="Grammar" {...scoreData.grammar} />}
            {scoreData.vocabulary && <ScorePill label="Vocabulary" {...scoreData.vocabulary} />}
            {scoreData.spelling && <ScorePill label="Spelling" {...scoreData.spelling} />}
            {scoreData.structure && <ScorePill label="Structure" {...scoreData.structure} />}
          </div>
