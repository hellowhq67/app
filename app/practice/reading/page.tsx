...
// Mock data for reading tasks
const READING_TASKS = [
  { id: "fib_rw", title: "Reading & Writing: Fill in Blanks", type: "reading_writing_fill_blanks", total: 100 },
  { id: "mcq_m", title: "Multiple Choice (Multiple)", type: "multiple_choice_multiple", total: 80 },
  { id: "ro", title: "Reorder Paragraphs", type: "reorder_paragraphs", total: 70 },
  { id: "fib_r", title: "Fill in Blanks (Reading)", type: "fill_in_blanks", total: 90 },
  { id: "mcq_s", title: "Multiple Choice (Single)", type: "multiple_choice_single", total: 60 },
];
...
export default function ReadingPracticePage() {
  ...
  const loadUserStats = async () => {
    ...
    try {
      // Load reading attempts from database
      const response = await fetch("/api/reading/attempts");
      if (response.ok) {
        const attempts = await response.json();

        // Process attempts to calculate stats
        const stats: UserReadingStats = {};

        READING_TASKS.forEach((task) => {
        ...
        <Card className="max-w-md mx-auto">
          <CardContent className="pt-6 text-center space-y-4">
            ...
            <p className="text-muted-foreground">
              Please sign in to access reading practice features
            </p>
  ...
  return (
    <PTELayoutClient>
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="bg-gradient-to-r from-primary/10 via-primary/5 to-transparent rounded-lg p-6 border border-primary/20">
          <div className="flex items-center justify-between">
            <div className="space-y-2">
              ...
              <p className="text-muted-foreground">
                Improve your reading skills with targeted practice questions.
              </p>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Reading Practice Tasks</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {READING_TASKS.map((task, index) => {
                ...
                return (
                  <div key={task.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/50 transition-colors">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <Button asChild>
                        <a href={`/pte/practice-new/reading/${task.type}`}>
                          Practice
                        </a>
                      </Button>
                    </div>
